...
public class ConfigManager {
	...
	private static void validateConfig() {
		if (config.getProtocol() == null) throw new EnhancedException("protocol is null");
		if (StringUtils.isBlank(config.getHost())) throw new EnhancedException("host is whitespace, empty or null");
		if (!Range.between(1024, 65535).contains(config.getPort())) throw new EnhancedException("port not in range 1024-65535 inclusive only");
	}
